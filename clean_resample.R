...
for (meas in list.files('measurements/')) {
  ...
  # Synchronize time - REMOVE WHEN SIGNALS SYNCHRONIZED (TIME SHIFT FOTR EMG SENSORS)
  emg_time_lag = grip_dat$measure_time[1] - emg_dat$measure_time[1]
  emg_dat <- emg_dat %>% 
    mutate(
      measure_time = measure_time + emg_time_lag
    )
